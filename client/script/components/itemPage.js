...
import { itemPageBtns } from '../utils/goodsButtons.js';

export default class ItemPage {
  constructor(itemPageSelector) {
    this.itemPageWrapper = document.querySelector(itemPageSelector);
  };
  ...
  onItemDataLoaded = () => {
    this.render();
    itemPageBtns(this.state.id);
  };

  loadItemData = async (itemId) => {
    const itemData = await getData.getItemById(itemId);
    this.state = itemData[0];
    this.onItemDataLoaded();
  };
  ...
  render() {
    ...
    this.itemPageWrapper.innerHTML = `
      ...
      <div class="good wrapper">
        <div class="good-images">
          ...
          <div class="good-image__item">
            ${secondImg ? `<img src=${secondImg} alt=${itemName}>` : ''}
          </div>
        </div>

        <div class=${count? 'good-item' : 'good-item not-in-stock'}>
          ${count >= settings.itemIsNewStockCount ? '<p class="good-item__new" style="display: block">Новинка</p>' : ''}
          <div class="good-item__section">
            ...
            <p class="good-item__price">
              ...
              ` : '<span class="good-item__empty" style="display: block">Нет в наличии</span>'}
            </p>
          </div>
          <div class="good-item__buttons">
            <button type="button" class="btn btn-good" data-count=${count} data-idd=${id} ${count? '' : 'style="display: none"'}>
